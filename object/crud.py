...
def upload_file_with_mime(s3_client, bucket_name, file_path: str) -> bool:
    try:
        mime = magic.Magic(mime=True)
        mime_type = mime.from_file(file_path)
        with open(file_path, 'rb') as f:
            s3_client.upload_fileobj(f, bucket_name, os.path.basename(file_path), ExtraArgs={'ContentType': mime_type})
        return True
